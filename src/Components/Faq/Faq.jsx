...
export default function AccordionWithIndex() {
    ...
    return (
        <div className="my-6">
            <h1 className="text-3xl text-center font-bold my-6" >FAQs Of StayOnCampus</h1>
            ...
            <div className=" max-w-[550px] cursor-pointer space-y-6">
                ...
                {dataArr.map((data, idx) => (
                    <div key={idx} onClick={() => handleToggle(idx)} className="flex items-center">
                        {/* the index div  */}
                        <div className="flex size-16 select-none items-center justify-center rounded-md bg-[#5a4044] text-2xl font-semibold text-white">
                            <span>0{idx + 1}</span>
                        </div>

                        <div className="relative h-[2px] w-10 bg-[#5a4044]">
                            <span className="absolute -left-2 -top-[5px] z-40 h-3 w-3 rounded-full border-2 border-[#5a4044] bg-white"></span>
                            <span className="h-1 w-10 bg-[#5a4044]"></span>
                            <span
                                className={`absolute -right-2 -top-[5px] z-40 h-3 w-3 rounded-full border-2 ${isOpen === idx ? 'border-[#5a4044] bg-white delay-100' : 'border-transparent'}`}
                            ></span>
                        </div>

                        {/* main accordion div  */}
                        <div className="text-center">
                            <div className="relative max-w-[450px] border-t-[12px] border-[#5a4044] bg-sky-50 p-3 shadow-md">
                                <span className="absolute right-0 top-0 h-0 w-0 border-b-[40px] border-r-[40px] border-b-transparent border-r-[#5a4044]"></span>
                                <h1 className="select-none text-lg text-[#5a4044]">{data.title}</h1>
                            </div>
                            <div className={`grid overflow-hidden text-slate-600 transition-all duration-300 ease-in-out ${isOpen === idx ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                                <div className="overflow-hidden">
                                    <div className="max-w-[450px] bg-[#5a4044] p-6 text-sm text-white">{data.description}</div>
                                </div>
                            </div>
                        </div>
                    </div>
